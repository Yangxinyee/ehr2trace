...
conversion fault the check suite in ``ehr2trace.validate`` is built for. Stated as an
...
Prerequisites (see ``tools/dqd/Dockerfile``)::
    ...
    docker build -t ehr2trace-dqd tools/dqd
...
from ehr2trace.config import DatasetConfig, load_dataset_config  # noqa: E402
from ehr2trace.faults import FAULTS, clone_work_tree  # noqa: E402
from ehr2trace.omop import build_omop  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--image", default="ehr2trace-dqd", help="the DQD image built from tools/dqd")
