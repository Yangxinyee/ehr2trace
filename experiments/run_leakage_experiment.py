...
#: The rule the converter publishes, and the baseline every increase is measured against.
BASE_ARM = "respects_availability"
...
def fit_and_score(cohort_path: Path, features_path: Path, arm: str, seed: int) -> dict:
    ...
    # The feature vocabulary is fixed by the training split of *this* arm, so a code that
    # only ever appears in a held-out patient is not a column the model was built around.
    # Sharing one vocabulary across arms would let the leaked arm's extra codes appear as
    # always-zero columns in the clean arm, which is a different experiment.
    ...
    return {
        ...
        "held_out_auroc": round(float(roc_auc_score(truth, score)), 4),
        "held_out_auprc": round(float(average_precision_score(truth, score)), 4),
    }, truth, score


#: Resamples behind every interval reported for this experiment.
BOOTSTRAP_RESAMPLES = 2000


def _intervals(truth, scores: dict, seed: int, resamples: int = BOOTSTRAP_RESAMPLES):
    """Percentile bootstrap intervals for each arm's AUROC and for the increases.

    One number per arm says which rule scored higher; it does not say whether the
    held-out set is large enough for the ordering to mean anything. Patients are
    resampled with replacement and every arm is scored on the *same* resample, so the
    increase over the clean arm is a paired quantity and its interval answers the
    question the figure asks. The generator is seeded from the run's own seed, which
    keeps the intervals part of the reproducible record.
    """
    import numpy as np
    ...
    n = len(truth)
    draws = {arm: [] for arm in scores}
    gains = {arm: [] for arm in scores if arm != BASE_ARM}
    for _ in range(resamples):
        idx = rng.integers(0, n, n)
        sample = truth[idx]
        if sample.min() == sample.max():
            continue
        scored = {arm: roc_auc_score(sample, s[idx]) for arm, s in scores.items()}
        for arm, value in scored.items():
            draws[arm].append(value)
        for arm in gains:
            gains[arm].append(scored[arm] - scored[BASE_ARM])
    ...
    return {arm: pct(v) for arm, v in draws.items()}, {arm: pct(v) for arm, v in gains.items()}
...
def main() -> None:
    ...
    for hours in [int(h) for h in args.horizons.split(",")]:
        ...
        scored = [fit_and_score(cohort, features, arm, args.seed) for arm in ARMS]
        arms = [row for row, _truth, _score in scored]
        truth = scored[0][1]
        scores = {row["arm"]: score for row, _truth, score in scored}
        arm_ci, gain_ci = _intervals(truth, scores, args.seed)
        for row in arms:
            row["held_out_auroc_ci"] = arm_ci[row["arm"]]
        by = {a["arm"]: a for a in arms}
        base = by[BASE_ARM]["held_out_auroc"]
        horizons.append({
            "horizon_hours": hours,
            ...
            "auroc_inflation": {
                ...
                for arm in ARMS if arm != BASE_ARM
            },
            "auroc_inflation_ci": gain_ci,
        })
