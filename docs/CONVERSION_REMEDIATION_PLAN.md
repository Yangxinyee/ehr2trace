...
| 2026-09-15 | MIMIC 对比文档把"零时长就诊 58 条"标为未达标，第 5 节的目标写的是"0 条 UNKNOWN" | 已达标，构建不改：58 条全部带真实就诊概念（9203 急诊、9201 住院），没有一条是 UNKNOWN 或 0。对比工具按原始条数判 `== 0`，比目标文字严，以本行为准 | 零时长来自交付本身：`admissions` 有 5 行 `admittime = dischtime`，`edstays` 有 53 行 `intime = outtime`，合计 58，与 OMOP 中的 58 条一一对应，转换没有造出任何一条。修复前是 1,139,334 条，其中 546,196 条发布为 UNKNOWN |
| 2026-09-15 | MIMIC 校验唯一未通过项 `UNIT_HOMOGENEOUS_PER_CODE`：12 个化验代码混用没有换算关系的单位 | 不自行处理，**待 Xinye 决定**。12 项里有 6 项只是占位符被当成一个单位族（`N/A`、`+/-`、`Pos/Neg` 与真实单位并列或彼此并列）；另外 6 项是真混用：51229 与 51228 的 `[U]/mL` 对 `[iU]/mL`、51249 的 `g/dL` 对 `%`、51099 的 `mg/mg` 对 `{ratio}`、227441 的 `mg/dL` 里混了 8 行 `[U]`、ED_PAIN 有 5 行疼痛评分带了时间单位 | 两种修法都会触发 MIMIC 全量重建：给参考表加别名或换算行会改参考摘要，作废规范层；`split_code_by_unit` 是源级配置，会改配置哈希，作废 ingest。阈值 `MIN_SECOND_UNIT_FAMILY_SHARE` 是 1%，调高它等于绕开这个发现，所以也没有动 |
| 2026-09-15 | JHU 校验唯一未通过项 `VISIT_CONCEPT_COVERAGE`：`visit_detail` 概念覆盖 3.01%（125,769 行中 3,782 行有概念），阈值为默认的 0.5 | 维持失败，不声明阈值。`datasets/ctpe.yaml` 的注释写明了这条约定："调低数字会掩盖真实缺口，缺口要在评审队列里关闭"。我一度把阈值声明为 0.02，随即撤回 | 缺口本身是 241 个 ADT 科室名只映射了 14 个（只有名称中确实写明 ICU 或 INTENSIVE CARE 的才映射，另有 8 个按楼栋楼层命名、标记为 ICU 的仍在评审队列）。另有 820,256 条 visit_detail 因无父就诊而不进 OMOP，规范层与 MEDS 全部保留 |
...
| 2026-09-17 | 干净计时与复现实验（`tools/measure_scale_cost.py`，空机、0 个游离 worker、12 worker、无 DuckDB 上限，克隆到 QVO 盘）：ingest 111 分钟（峰值 9.6 GB）、identity 13 秒（47.9 GB）、canonical 191 分钟（167.2 GB）、omop 47 分钟（230.9 GB）；identity 与 canonical 的重建摘要与保存层一致，但最初两次 omop、meds 重建都报「与原层不同」，且不能用 `cdm_source` 解释（两库 `cdm_release_date` 同为 09-16） | 逐表比对找到根因：重建库 `type_concept` 全为 0、`term_map` 少 893 行 SOURCE——映射注册表默认取**当前目录**下的 `mappings/`，而计时工具从论文仓库运行，`MappingRegistry.load` 对不存在的目录静默返回空注册表。转换器本身是确定的：术语解析在两个不同哈希种子的进程中结果完全一致，并与保存层 MEDS 编码表逐概念计数相同。修复 `b6ec7aa`（找不到注册表即以 `MAPPINGS_DIR_MISSING` 拒绝构建；OMOP 运行记录与 MEDS 构建报告写明注册表路径与条数）与论文仓库 `cf790f3`（计时工具自己设置 `EHR_MAPPINGS_DIR` 并记入 `cost.json`）。指定注册表后重跑：omop 全部表与保存层一致，只有记录构建日期的 `cdm_source` 不同；meds 重跑 128 分钟（峰值 136.8 GB），摘要与保存层完全一致——四层全部复现；数字与 `cost.json` 已进论文 §4.6（论文仓库 4b5612f） | 排除过的解释：DuckDB 1.5.5 在 `preserve_insertion_order=false` 下 6 千万行 `COPY … ORDER BY` 仍有序；保存层分片每片一名患者、按患者号严格有序、无空号。教训：注册表是和词表同级的输入，必须显式指定；重建「不同」时先比 `type_concept` 与 `term_map` 的行数 |
| 2026-09-17 | 论文表格与图核对：故障矩阵图和架构图的校验分组是旧文件（19 个故障、缺"声明与参考表"一组，正文已是 28 个故障、55 项校验）；保证表还有两行"时间策略更新前"的历史对比；对比表的校验行是 09-12 演示子集的 40 项旧套件 | 用生成器重做全部表图，删去历史行，改掉图注里"漏检后新增"的叙事。用当前软件重跑 100 名患者的公开演示子集（住院 + ICU 模块，971,545 事件，54 项校验 45 通过 / 8 跳过 / 1 失败，失败项与全量相同的 `UNIT_HOMOGENEOUS_PER_CODE`），两份已发布基线用早先会话保留的下载副本（校验和一致，已存到 `ehr2trace_demo/baselines`）。演示子集的校验暴露了 `b6ec7aa` 的一处错误：`validate.py` 里 `UNDECIDED_PROPOSALS_NEVER_PUBLISHED` 的局部导入被改错，检查抛 `NameError` 而单元测试全绿——已修，并加了 pyflakes 未定义名回归测试 | 对比表现在三种转换读同样的模块；我方概念覆盖率 18.9%（ICU 监护项目未映射），论文如实说明。教训：改 validate/omop/meds 后跑一次演示子集全流程（3 分钟）再提交 |
...
- ~~论文的运行时数字仍需一次干净的计时运行~~ 已完成（2026-09-16/17，见 9.3）：ingest 111、canonical 191、omop 47、meds 128 分钟，记录在 `ehr2trace-paper/results/cost.json`。
